export class SafeToken {
  ...
  constructor(init?: { TokenTime?: number; RefreshDays: number }) {
    ...
    this.tokenT = init?.TokenTime || 600;
  ...
  newToken(_r?: true) {
    ...
    setTimeout(() => {
      const diff = this.timeDiff(this.lastAccessTime);
  ...
  getRefreshToken() {
    const diff = this.timeDiff(this.lastrefreshTime);
...
console.log(
  Auth.verifyToken(Auth.newToken()),
  Auth.verifyRefreshToken(Auth.getRefreshToken())
);
