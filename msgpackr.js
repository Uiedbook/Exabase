(function (global, factory) {
    ...
    if (token < 0xa0) {
      ...
      if (srcStringEnd >= position$1) {
        return srcString.slice(
          position$1 - srcStringStart,
          (position$1 += length) - srcStringStart
        );
      }
      ...
      switch (token) {
        ...
        case 0xc1:
          if (bundledStrings$1) {
            ...
            if (value > 0)
              return bundledStrings$1[1].slice(
                bundledStrings$1.position1,
                (bundledStrings$1.position1 += value)
              );
            else
              return bundledStrings$1[0].slice(
                bundledStrings$1.position0,
                (bundledStrings$1.position0 -= value)
              );
          }
        ...
        case 0xca:
          ...
          if (currentUnpackr.useFloat32 > 2) {
            ...
            let multiplier =
              mult10[
                ((src[position$1] & 0x7f) << 1) | (src[position$1 + 1] >> 7)
              ];
            position$1 += 4;
            return (
        ...
        case 0xd9:
          ...
          if (srcStringEnd >= position$1) {
            return srcString.slice(
              position$1 - srcStringStart,
              (position$1 += value) - srcStringStart
            );
          }
  ...
  function readExt(length) {
    ...
    if (currentExtensions[type]) {
      ...
      return currentExtensions[type](
        src.subarray(position$1, (end = position$1 += length)),
        (readPosition) => {
    ...
    constructor(options) {
      ...
      const pack = (value) => {
        ...
        if (type === "string") {
          ...
          if (value >>> 0 === value) {
            ...
            if (
              value < 0x20 ||
              (value < 0x80 && this.useRecords === false) ||
              (value < 0x40 && !this.randomAccessStructure)
            ) {
          ...
          if (
            ...
            if (this.largeBigIntToFloat) {
              ...
              throw new RangeError(
                value +
                  " was too large to fit in MessagePack 64-bit integer format, use" +
              ...
              for (let key in object) {
                if (safePrototype || object.hasOwnProperty(key)) {
                  ...
                  if (nextTransition) transition = nextTransition;
                    ...
                    } // otherwise we need to insert the record, moving existing data after the record
                    else
                      insertNewRecord(
                        transition,
                        keys,
      ...
      const newRecord = (transition, keys, newTransitions) => {
        ...
        let highByte = (keys.highByte =
          recordId >= 0x60 && useTwoByteRecords ? (recordId - 0x60) >> 5 : -1);
      ...
      const writeStruct = (object, safePrototype) => {
        ...
        if (newPosition === 0)
          // bail and go to a msgpack object
  ...
  function writeBundles(start, pack, incrementPosition) {
    if (bundledStrings.length > 0) {
      targetView.setUint32(
        ...
        position + incrementPosition - bundledStrings.position - start
      );
  ...
  function prepareStructures(structures, packr) {
    structures.isCompatible = (existingStructures) => {
      ...
      if (!compatible)
        // we want to merge these existing structures immediately since we already have it and we are in the right transaction
